    ...
    <div class="card-body">
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <h6 class="m-0 text-primary"><span class="font-weight-bold">Thêm hoặc sửa </span> danh mục</h6>
            </div>
            <div class="row">
                <div class="col-md-6 ">
                    <form class="m-4" method="POST" action="{{ route('category.update',$category->id)}}" enctype="multipart/form-data" >
                        @csrf @method('PUT')
                            <input type="text" name="id" value="{{$category->id}}" hidden>
                        <div class=" form-group">
                            <label class="font-weight-bold text-success" for="name">Tên danh mục </label>
                            <input type="text" class=" form-control border border-success" id="name" name="name" value="{{(old('name'))?old('name'):$category->name}}">
                            @error('name')
                        ...
                        <div class=" form-group">
                            ...
                            <div class="form-check">
                                ...
                                @if(($category->status==1&&old('status')=='')||old('status')==1)
                                  {{'checked'}}
                                @endif>
                            ...
                            <div class="form-check">
                                ...
                                @if(($category->status==0&&old('status')=='')||(old('status')==0&&old('status')!=''))
                                  {{'checked'}}
                                @endif>
                        ...
                        <div class=" form-group">
                            ...
                            <input type="text" class="form-control border border-success" name="slug" id="slug" value="{{(old('slug'))?old('slug'):$category->slug}}">
                            @error('slug')
                                <small class="help-block text-danger">{{$message}}</small>
                            @enderror
                        </div>
                        ...
                        <div class="form-group"> 
                            <img src="{{url('public')}}/uploads/logos/{{$category->logo}}" alt="" style="max-height: 150px">
                        </div>  
